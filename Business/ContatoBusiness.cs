...
namespace Contato.Business
{
    public class ContatoBusiness
    {
            ...
            public AddressBook GetContatos()
            {
                return AddressBook;
            }
            ...
            public Contact criarContato(string nome, int numero)
            {
                try
                {
                    if (string.IsNullOrEmpty(nome) || nome.Length < 3) throw new InternalException(System.Net.HttpStatusCode.BadRequest, "Nome inválido", "O nome não atende os requisitos básicos");
            ...
            public string editarContato(int numero, string novoNome)
            {
                try
                {
                    if (string.IsNullOrEmpty(novoNome) || novoNome.Length < 3) throw new InternalException(System.Net.HttpStatusCode.BadRequest, "Nome inválido", "O nome não atende os requisitos básicos");
